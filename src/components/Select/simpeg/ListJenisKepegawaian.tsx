...
import { useGetJenisKepegawaianQuery } from '@/store/slices/referensiAPI'
...
export function SelectListJenisKepegawaian({
  name,
  headerLabel,
  placeholder,
  isDisabled,
  ...
  const [listJenisKepegawaian, setListJenisKepegawaian] = useState<string[]>([])
  ...
  useEffect(() => {
    if (!isFetching) {
      if (data?.meta?.page > 1) {
        setListJenisKepegawaian((prevData) => [
          ...prevData,
          ...(data?.data ?? []),
        ])
      } else {
        setListJenisKepegawaian([...(data?.data ?? [])])
      }
    }
  }, [data])

  let JenisKepegawaianOption = []
  if (isSuccess) {
    JenisKepegawaianOption = listJenisKepegawaian.map((item) => {
      return {
        value: item,
        label: item,
      }
    ...
    <FormField
      ...
      render={({ field }) => {
        return (
          <FormItem
            className={cn(
              'z-50 flex w-full items-center gap-12 text-[2rem] text-warna-dark phones:flex-col phones:items-start phones:gap-12 phones:text-[2.4rem]',
              className,
            )}
          >
            {headerLabel && (
            ...
            <div className="w-3/5 phones:w-full">
              <FormControl>
                <Select
                  ...
                  options={JenisKepegawaianOption}
                  value={
                    JenisKepegawaianOption.filter(
