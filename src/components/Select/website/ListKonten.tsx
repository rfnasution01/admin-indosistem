...
import { useGetListKontenQuery } from '@/store/slices/referensiAPI'
...
export function SelectListKonten({
  name,
  headerLabel,
  placeholder,
  isDisabled,
  ...
  const [listKonten, setListKonten] = useState<ReferensiType[]>([])

  const isJenis =
    jenis === 'Route'
      ? 'referensi/route_menu'
      : jenis === 'KategoriBerita'
        ? 'referensi/kategori/berita'
        : jenis === 'KategoriAgenda'
          ? 'referensi/kategori/agenda'
          : jenis === 'KategoriPengumuman'
            ? 'referensi/kategori/pengumuman'
            : jenis === 'KategoriPrestasi'
              ? 'referensi/kategori/prestasi'
              : jenis === 'KategoriMading'
                ? 'referensi/kategori/mading'
                : null

  const { data, isSuccess, isLoading, isFetching } = useGetListKontenQuery(
    {
      jenis: isJenis,
    },
    { skip: !isJenis },
