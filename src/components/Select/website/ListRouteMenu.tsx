...
import { useGetRouteMenuQuery } from '@/store/slices/referensiAPI'
...
export function SelectListRouteMenu({
  name,
  headerLabel,
  placeholder,
  isDisabled,
  ...
  const [listRouteMenu, setListRouteMenu] = useState<ReferensiType[]>([])

  const { data, isSuccess, isLoading, isFetching } = useGetRouteMenuQuery()

  useEffect(() => {
    if (!isFetching) {
      if (data?.meta?.page > 1) {
        setListRouteMenu((prevData) => [...prevData, ...(data?.data ?? [])])
      } else {
        setListRouteMenu([...(data?.data ?? [])])
      }
    }
  }, [data])
  ...
  let RouteMenuOption = []
  if (isSuccess) {
    RouteMenuOption = listRouteMenu.map((item) => {
      return {
        value: item?.id,
        label: item?.nama,
      }
