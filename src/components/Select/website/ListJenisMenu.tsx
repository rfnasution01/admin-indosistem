...
export function SelectListJenisMenu({
  name,
  headerLabel,
  placeholder,
  isDisabled,
  ...
  const [listJenisMenu, setListJenisMenu] = useState<string[]>([])

  const { data, isSuccess, isLoading, isFetching } = useGetJenisMenuQuery()

  useEffect(() => {
    if (!isFetching) {
      if (data?.meta?.page > 1) {
        setListJenisMenu((prevData) => [...prevData, ...(data?.data ?? [])])
      } else {
        setListJenisMenu([...(data?.data ?? [])])
      }
    }
  }, [data])
  ...
  let JenisMenuOption = []
  if (isSuccess) {
    JenisMenuOption = listJenisMenu.map((item) => {
      return {
        value: item,
        label: item,
      }
            ...
            <div className="w-full phones:w-full">
              <FormControl>
                <Select
                  ...
                  options={JenisMenuOption}
                  value={
                    JenisMenuOption.filter(
