...
import { useGetBeritaQuery } from '@/store/slices/referensiAPI'
...
export function SelectListBerita({
  name,
  headerLabel,
  placeholder,
  isDisabled,
  ...
  const [listBerita, setListBerita] = useState<ReferensiType[]>([])

  const { data, isSuccess, isLoading, isFetching } = useGetBeritaQuery()

  useEffect(() => {
    if (!isFetching) {
      if (data?.meta?.page > 1) {
        setListBerita((prevData) => [...prevData, ...(data?.data ?? [])])
      } else {
        setListBerita([...(data?.data ?? [])])
      }
    }
  }, [data])
  ...
  let BeritaOption = []
  if (isSuccess) {
    BeritaOption = listBerita.map((item) => {
      return {
        value: item?.id,
        label: item?.nama,
      }
            ...
            <div className="w-full phones:w-full">
              <FormControl>
                <Select
                  ...
                  options={BeritaOption}
                  value={
                    BeritaOption.filter((item) => item.value === field.value)[0]
                  }
