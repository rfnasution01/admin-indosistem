...
export function DashboardChart({
  ...
  function hitungTotalJlh(data: GetChartType[]): number {
    return data?.reduce((total, item) => total + parseInt(item?.jlh), 0)
  }
  return (
    <div className="flex h-full w-full flex-col gap-32 rounded-3x border border-[#E0E0E0] p-32 phones:h-auto phones:w-full">
              ...
              {item?.slice(0, 4)?.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex w-full items-center justify-between gap-12">
                    ...
                    <p className="font-roboto">{item?.jlh}</p>
                  </div>
                </div>
              ))}
            </div>
                  ...
                  >
                    {item?.length > 8 && idx === 3 ? (
                      ...
                      <>
                        ...
                        />
                        <div className="flex w-full items-center justify-between gap-12">
                          <p className="line-clamp-1">{list?.nama}</p>
                          <p className="font-roboto">{list?.jlh}</p>
                        </div>
                      </>
                    )}
                  </div>
          ...
          {item?.length > 0 && (
            <div className="flex flex-col gap-8">
              ...
              <div className="flex items-center justify-between gap-12">
                <p>Total</p>
                <p className="font-roboto">{hitungTotalJlh(item)}</p>
              </div>
            </div>
          )}
        </div>
